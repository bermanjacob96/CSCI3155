  ...
  def eval(env: Env, e: Expr): Expr = {
    e match {
      ...
      case Unary(Neg, e1) => {
        ...
        val np = - toNumber(v1)
        N(np)
        // N(-toNumber(eval(env,e1)))
      }

        // EvalAnd
      case Binary(And, e1, e2) => {
        // NOOOOOOOOOO if (toBoolean(eval(env,e1))) { eval(env,e2) } else { eval(env,e1) }
        val v1:Expr = eval(env,e1)
        // EvalAndTrue
        val b1:Boolean = toBoolean(v1)
        if(b1) {
  ...
  def substitute(e: Expr, v: Expr, x: String): Expr = {
    ...
    e match {
      ...
      case Binary(bop, e1, e2) => {
        val e1Subs = substitute(e1, v, x)
        val e2Subs = substitute(e2, v, x)
        Binary(bop,e1Subs,e2Subs)
      }
      case If(e1, e2, e3) => ???
      case Call(e1, e2) => ???
      case Var(y) => ???
  ...
  def step(e: Expr): Expr = {
    e match {
      ...
      case Binary(And, v1, e2) if isValue(v1) & toBoolean(v1) => e2
        // DoAndFalse
      case Binary(And, v1, e2) if isValue(v1) => v1
